    ...
    if data.shape[-1] == solution.neq:
        ...
        for i in range(solution.nphys):
            ...
            if phys_variable == b"rho":
                ...
                solution_phys[:, i] = calculate_u_cons(
                    data_loc,
                ...
                solution_phys[:, i] = calculate_cs_cons(
                    data_loc,
                    solution.parameters["adimensionalization"]["speed_scale"],
                    solution.metadata.indices.conservative,
                )
            elif phys_variable == b"M":
                solution_phys[:, i] = calculate_M_cons(
                    data_loc, solution.metadata.indices.conservative
                )
            elif phys_variable == b"rhon":
                solution_phys[:, i] = calculate_nn_cons(
                    data_loc,
                    solution.parameters["adimensionalization"]["density_scale"],
                    solution.metadata.indices.conservative,
                )
            elif phys_variable == b"k":
                solution_phys[:, i] = calculate_k_cons(
                    data_loc,
                    solution.parameters["adimensionalization"]["speed_scale"] ** 2,
                    solution.metadata.indices.conservative,
                )
            elif phys_variable == b"epsilon":
                solution_phys[:, i] = calculate_eps_cons(
                    data_loc,
                    solution.parameters["adimensionalization"]["speed_scale"] ** 2
                ...
                raise KeyError(
                    "Unknown variable, go into the code and add this variable if you are sure"
                )
        ...
        if len(data.shape) == 4:
            grad_phys = np.zeros(
                (data.shape[0] * data.shape[1], solution.nphys, solution.ndim)
            )
            data_loc = data.reshape(
                (data.shape[0] * data.shape[1], solution.neq, solution.ndim)
            )
            if data is solution.views.gauss.gradient.conservative:
                sol_loc = solution.views.gauss.solution.conservative.reshape(
                    (data.shape[0] * data.shape[1], solution.neq)
                )
            else:
                sol_loc = solution.views.glob.solution.conservative.reshape(
                    (data.shape[0] * data.shape[1], solution.neq)
                )
...
def _assign_physical_solution(solution, data, solution_phys):
    if data is solution.views.glob.solution.conservative or (
        ...
        solution.views.glob.solution.physical = solution_phys.reshape(
            (data.shape[0], data.shape[1], solution.nphys)
        )
    elif data is solution.views.gauss.solution.conservative:
        solution.views.gauss.solution.physical = solution_phys.reshape(
            (data.shape[0], data.shape[1], solution.nphys)
        )
    elif len(data.shape) == 2:
...
def _assign_physical_gradient(solution, data, grad_phys):
    if data is solution.views.glob.gradient.conservative or (
        ...
        solution.views.glob.gradient.physical = grad_phys.reshape(
            (data.shape[0], data.shape[1], solution.nphys, solution.ndim)
        )
    elif data is solution.views.gauss.gradient.conservative:
        solution.views.gauss.gradient.physical = grad_phys.reshape(
            (data.shape[0], data.shape[1], solution.nphys, solution.ndim)
        )
    elif len(data.shape) == 3:
