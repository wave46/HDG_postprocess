...
cdef class Exact2DMeshFunction:
    ...
    def __init__(
        ...
        node_leaf_count = []
        leaf_indices = []
        element_ids = np.arange(unique_values.shape[0], dtype=np.int32)
        self._build_tree(
            element_ids,
            node_bounds,
            node_left,
    ...
    cdef INDEX_t _build_tree(
        ...
        cdef object element_ids_arr = np.ascontiguousarray(element_ids_obj, dtype=np.int32)
        cdef INDEX_t[::1] element_ids = element_ids_obj
        ...
        split_values = np.ascontiguousarray(np.asarray(self._element_centers)[element_ids_arr, axis], dtype=np.float64)
    ...
    def __call__(self, double x, double y):
        ...
        if x < node_bounds[0, 0] or x > node_bounds[0, 2] or y < node_bounds[0, 1] or y > node_bounds[0, 3]:
        ...
        while stack_size > 0:
            ...
            if x < node_bounds[node_id, 0] or x > node_bounds[node_id, 2] or y < node_bounds[node_id, 1] or y > node_bounds[node_id, 3]:
                continue

            if node_leaf_count[node_id] > 0:
                for leaf_idx in range(node_leaf_start[node_id], node_leaf_start[node_id] + node_leaf_count[node_id]):
                    elem_id = leaf_indices[leaf_idx]
                    if x < boxes[elem_id, 0] or x > boxes[elem_id, 2] or y < boxes[elem_id, 1] or y > boxes[elem_id, 3]:
                        continue
                    ...
                    while tri_id < tri_end:
                        if self._point_in_triangle(vertices, connectivity, tri_id, x, y):
                            ...
                            self._cached_x = x
                            self._cached_y = y
                            self._cached_value = float(element_values[elem_id])
