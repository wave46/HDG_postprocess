...
class HDGsolution:
    ...
    def __init__(
        ...
        for raw_solution, raw_solution_skeleton, raw_gradient in zip(
            ...
            raw_gradient = raw_gradient.reshape(
                raw_gradient.shape[0], self.neq, self.ndim
            )
    ...
    def dnn_parameters(self, value):
        ...
        self._dnn_parameters["dnn_max_adim"] = (
            self._dnn_parameters["dnn_max"]
            / self.parameters["adimensionalization"]["length_scale"] ** 2
            * self.parameters["adimensionalization"]["time_scale"]
        )
        if not value["const"]:
            self._dnn_parameters["dnn_min_adim"] = (
                self._dnn_parameters["dnn_min"]
                / self.parameters["adimensionalization"]["length_scale"] ** 2
                * self.parameters["adimensionalization"]["time_scale"]
            )
    ...
    def dk_parameters(self, value):
        ...
        self._dk_parameters["dk_max_adim"] = (
            self._dk_parameters["dk_max"]
            / self.parameters["adimensionalization"]["length_scale"] ** 2
            * self.parameters["adimensionalization"]["time_scale"]
        )

        self._dk_parameters["dk_min_adim"] = (
            self._dk_parameters["dk_min"]
            / self.parameters["adimensionalization"]["length_scale"] ** 2
            * self.parameters["adimensionalization"]["time_scale"]
        )
    ...
    def recombine_full_solution(self):
        ...
        self._solution_glob = np.zeros(
            (
                self.mesh._nelems_glob,
                self.mesh.mesh_parameters["nodes_per_element"],
                self.neq,
            )
        )
        self._gradient_glob = np.zeros(
            (
                self.mesh._nelems_glob,
                self.mesh.mesh_parameters["nodes_per_element"],
                self.neq,
        ...
        self._magnetic_field_glob = np.zeros(
            (self.mesh._nelems_glob, self.mesh.mesh_parameters["nodes_per_element"], 3)
        )
        self._poloidal_flux_glob = np.zeros(
            (self.mesh._nelems_glob, self.mesh.mesh_parameters["nodes_per_element"])
        )
        if self.parameters["switches"]["ohmicsrc"][0] == 1:
            self._jtor_glob = np.zeros(
                (self.mesh._nelems_glob, self.mesh.mesh_parameters["nodes_per_element"])
            )

        for i in range(self.n_partitions):
            # reshape to the shape of elements
            raw_solution = self.raw_solutions[i].reshape(
                self.raw_solutions[i].shape[0]
                // self.mesh.mesh_parameters["nodes_per_element"],
                self.mesh.mesh_parameters["nodes_per_element"],
                self.neq,
            )
            ...
            raw_gradient = self.raw_gradients[i].reshape(
                self.raw_gradients[i].shape[0]
                // self.mesh.mesh_parameters["nodes_per_element"],
                self.mesh.mesh_parameters["nodes_per_element"],
                self.neq,
                self.ndim,
            ...
            # magnetic field
            raw_field = self.raw_equilibriums[i]["magnetic_field"][
                self.mesh.raw_connectivity[i]
            ]

            # poloidal flux
            raw_poloidal_flux = self.raw_equilibriums[i]["poloidal_flux"][
                self.mesh.raw_connectivity[i]
            ]

            if self.parameters["switches"]["ohmicsrc"][0] == 1:
                # plasma current
                raw_jtor = self.raw_equilibriums[i]["plasma_current"][
                    self.mesh.raw_connectivity[i]
                ]
            ...
            if self.n_partitions > 1:
                ...
                raw_solution = raw_solution[
                    ~self.mesh.raw_ghost_elements[i].astype(bool).flatten(), :
                ]
                self._solution_glob[
                    self.mesh.raw_rest_mesh_data[i]["loc2glob_el"][
                        ~self.mesh.raw_ghost_elements[i].flatten()
                    ]
                ] = raw_solution
                ...
                raw_poloidal_flux = raw_poloidal_flux[
                    ~self.mesh.raw_ghost_elements[i].astype(bool).flatten()
                ]
        ...
        self._magnetic_field_unit_glob = (
            self._magnetic_field_glob
            / np.sqrt((self._magnetic_field_glob**2).sum(axis=-1))[:, :, None]
        )
    ...
    def recombine_simple_full_solution(self):
        ...
        self._solution_simple[self.mesh.connectivity_glob.reshape(-1, 1).ravel(), :] = (
            self.solution_glob.reshape(
                self.solution_glob.shape[0] * self.solution_glob.shape[1], self.neq
            )
        )

        self._gradient_simple = np.zeros(
            [self.mesh.vertices_glob.shape[0], self.neq, self.ndim]
        )
        self._gradient_simple[
            self.mesh.connectivity_glob.reshape(-1, 1).ravel(), :, :
        ] = self.gradient_glob.reshape(
            self.gradient_glob.shape[0] * self.gradient_glob.shape[1],
            self.neq,
            self.ndim,
        )

        self._magnetic_field_simple = np.zeros([self.mesh.vertices_glob.shape[0], 3])
        self._magnetic_field_simple[
            self.mesh.connectivity_glob.reshape(-1, 1).ravel(), :
        ] = self.magnetic_field_glob.reshape(
            self.magnetic_field_glob.shape[0] * self.magnetic_field_glob.shape[1], 3
        )

        self._poloidal_flux_simple = np.zeros([self.mesh.vertices_glob.shape[0]])
        self._poloidal_flux_simple[
            self.mesh.connectivity_glob.reshape(-1, 1).ravel()
        ] = self.poloidal_flux_glob.reshape(
            self.poloidal_flux_glob.shape[0] * self.poloidal_flux_glob.shape[1]
        )
    ...
    def recombine_boundary_solution(self):
        ...
        for key in self.mesh._connectivity_b_glob.keys():
            ...
            for face_element_number, face_local_number in zip(
                self.mesh.face_element_number[key], self.mesh.face_local_number[key]
            ):
                self._solution_boundary[key].append(
                    self.solution_glob[
                        face_element_number,
                        self.mesh.reference_element["faceNodes"][face_local_number, :],
                        :,
                    ]
                )
                self._gradient_boundary[key].append(
                    self.gradient_glob[
                        face_element_number,
                        self.mesh.reference_element["faceNodes"][face_local_number, :],
                        :,
                ...
                self._magnetic_field_boundary[key].append(
                    self.magnetic_field_glob[
                        face_element_number,
                        self.mesh.reference_element["faceNodes"][face_local_number, :],
                        :,
                    ]
                )
                self._magnetic_field_unit_boundary[key].append(
                    self.magnetic_field_unit_glob[
                        face_element_number,
                        self.mesh.reference_element["faceNodes"][face_local_number, :],
                        :,
                    ]
        ...
        if self.n_partitions > 1:
            solution_skeleton_boundary = np.ones(
                (
                    self.mesh._nfaces_glob,
                    self.mesh.mesh_parameters["nodes_per_face"],
                    self.neq,
                )
            )
            for i in range(self.n_partitions):
                non_ghost = ~self.mesh.raw_ghost_faces[i].flatten()
                raw_solution = self.raw_solutions_skeleton[i].reshape(
                    self.raw_solutions_skeleton[i].shape[0]
                    // self.mesh.mesh_parameters["nodes_per_face"],
                    self.mesh.mesh_parameters["nodes_per_face"],
                    self.neq,
                )
                solution_skeleton_boundary[
                    self.mesh.raw_rest_mesh_data[i]["loc2glob_fa"][:][non_ghost], :
                ] = raw_solution[non_ghost]
            solution_skeleton_boundary = solution_skeleton_boundary[
                self.mesh._filled, :, :
            ]
        else:
            solution_skeleton_boundary = self.raw_solutions_skeleton[0].reshape(
                self.raw_solutions_skeleton[0].shape[0]
                // self.mesh.mesh_parameters["nodes_per_face"],
                self.mesh.mesh_parameters["nodes_per_face"],
                self.neq,
            )
            solution_skeleton_boundary = solution_skeleton_boundary[
                -self.mesh.raw_mesh_numbers[0]["Nextfaces"] :, :, :
            ]
        self._solution_skeleton_boundary = {}
        ...
        for key, indices in self.mesh._indices.items():
            ...
            for ind in indices:
                self._solution_skeleton_boundary[key].append(
                    solution_skeleton_boundary[ind, :, :]
                )
    ...
    def calculate_in_gauss_points(self):
        ...
        self._solution_gauss = np.einsum(
            "ij,kjh->kih", self.mesh.reference_element["N"], self.solution_glob
        )
        self._gradient_gauss = np.einsum(
            "ij,kjhl->kihl", self.mesh.reference_element["N"], self.gradient_glob
        )
        self._magnetic_field_gauss = np.einsum(
            "ij,kjh->kih", self.mesh.reference_element["N"], self.magnetic_field_glob
        )
        self._magnetic_field_unit_gauss = np.einsum(
            "ij,kjh->kih",
            self.mesh.reference_element["N"],
            self.magnetic_field_unit_glob,
        )
        self._jtor_gauss = np.einsum(
            "ij,kj->ki", self.mesh.reference_element["N"], self.jtor_glob
        )
    ...
    def calculate_in_boundary_gauss_points(self, boundaries):
        ...
        boundary_ordering, connectivity_ordered = self.mesh.calculate_gauss_boundary(
            boundaries, self.raw_solution_boundary_infos
        )

        solution_boundary_ordered = np.empty(
            [
                0,
                self.solution_boundary[boundaries[0]][0].shape[1],
                self.solution_boundary[boundaries[0]][0].shape[2],
            ]
        )
        solution_skeleton_boundary_ordered = np.empty(
            [
                0,
                self.solution_skeleton_boundary[boundaries[0]][0].shape[1],
                self.solution_skeleton_boundary[boundaries[0]][0].shape[2],
            ]
        )
        gradient_boundary_ordered = np.empty(
            [
                0,
                self._gradient_boundary[boundaries[0]][0].shape[1],
                self._gradient_boundary[boundaries[0]][0].shape[2],
                self._gradient_boundary[boundaries[0]][0].shape[3],
            ]
        )
        magnetic_field_boundary_ordered = np.empty(
            [
                0,
                self._magnetic_field_boundary[boundaries[0]][0].shape[1],
                self._magnetic_field_boundary[boundaries[0]][0].shape[2],
            ]
        )
        magnetic_field_unit_boundary_ordered = np.empty(
            [
                0,
                self._magnetic_field_unit_boundary[boundaries[0]][0].shape[1],
                self._magnetic_field_unit_boundary[boundaries[0]][0].shape[2],
            ]
        )
        for bound_ordering in boundary_ordering:
            solution_boundary_ordered = np.vstack(
                [
                    solution_boundary_ordered,
                    self.solution_boundary[boundaries[bound_ordering[0]]][
                        bound_ordering[1]
                    ],
                ]
            )
            solution_skeleton_boundary_ordered = np.vstack(
                [
                    ...
                    self.solution_skeleton_boundary[boundaries[bound_ordering[0]]][
                        bound_ordering[1]
                    ],
                ]
            )
            gradient_boundary_ordered = np.vstack(
                [
                    gradient_boundary_ordered,
                    self._gradient_boundary[boundaries[bound_ordering[0]]][
                        bound_ordering[1]
                    ],
                ]
            )
            magnetic_field_boundary_ordered = np.vstack(
                [
                    magnetic_field_boundary_ordered,
                    self._magnetic_field_boundary[boundaries[bound_ordering[0]]][
                        bound_ordering[1]
                    ],
                ]
            )
            magnetic_field_unit_boundary_ordered = np.vstack(
                [
                    ...
                    self._magnetic_field_unit_boundary[boundaries[bound_ordering[0]]][
                        bound_ordering[1]
                    ],
                ]
            )

        self._solution_boundary_gauss = np.einsum(
            "ij,kjh->kih", self.mesh.reference_element["N1d"], solution_boundary_ordered
        )
        self._solution_skeleton_boundary_gauss = np.einsum(
            "ij,kjh->kih",
            self.mesh.reference_element["N1d"],
            solution_skeleton_boundary_ordered,
        )
        self._gradient_boundary_gauss = np.einsum(
            "ij,kjhl->kihl",
            self.mesh.reference_element["N1d"],
            gradient_boundary_ordered,
        )
        self._magnetic_field_boundary_gauss = np.einsum(
            "ij,kjh->kih",
            self.mesh.reference_element["N1d"],
            magnetic_field_boundary_ordered,
        )
        self._magnetic_field_unit_boundary_gauss = np.einsum(
            "ij,kjh->kih",
            self.mesh.reference_element["N1d"],
            magnetic_field_unit_boundary_ordered,
        )
    ...
    def plot_overview(self, n_levels=100):
        ...
        q0, n0, E0, l0, m0, v0, T0, t0 = self.get_main_scales()
        ...
        for i in range(self.neq):
            ...
            if (cons_variable != b"Gamma") and (cons_variable != b"k"):
                axes[i // 2, i % 2] = self.mesh.plot_full_mesh(
                    solutions_dimensional[:, i],
                    ax=axes[i // 2, i % 2],
                    log=True,
                    label=colorbar_labels[i],
                    connectivity=self.mesh.connectivity_big,
                    n_levels=n_levels,
                )
            else:
                axes[i // 2, i % 2] = self.mesh.plot_full_mesh(
    ...
    def plot_overview_difference(self, second_solution, n_levels=100):
        ...
        difference_dimensional = (
            self.solution_simple.copy() - second_solution.solution_simple.copy()
        )
        colorbar_labels = []
        # in fact for dimensionalization there is field 'reference_values_conservative_variables' but it's not completely correct
        for i in range(self.neq):
            cons_variable = self.parameters["physics"]["conservative_variable_names"][i]
            if cons_variable == b"rho":
                difference_dimensional[:, i] *= n0
                colorbar_labels.append(r"n, m$^{-3}$")
            elif cons_variable == b"Gamma":
                difference_dimensional[:, i] *= n0 * v0
                colorbar_labels.append(r"$\Gamma$, m$^{-2}$ s$^{-1}$")
            elif cons_variable == b"nEi":
                difference_dimensional[:, i] *= n0 * E0
                colorbar_labels.append(r"nE$_i$, m$^{-1}$ s$^{-2}$")
            elif cons_variable == b"nEe":
                difference_dimensional[:, i] *= n0 * E0
                colorbar_labels.append(r"nE$_e$, m$^{-1}$ s$^{-2}$")
    ...
    def get_main_scales(self):
        """Unpack main dimension scales"""
        adim = self.parameters["adimensionalization"]
        keys = [
            "charge",
            "density",
            "specific_energy",
        ...
        q0, n0, E0, l0, m0, v0, T0, t0 = [adim[k + "_scale"] for k in keys]
        return q0, n0, E0, l0, m0, v0, T0, t0
    ...
    def cons2phys(self, data):
        ...
        # Alises
        q0, n0, E0, l0, m0, v0, T0, t0 = self.get_main_scales()
        Mref = self.parameters["physics"]["Mref"]
        ...
        if data.shape[-1] == self.neq:
            ...
            for i in range(self.nphys):
                ...
                if phys_variable == b"rho":
                    ...
                    solution_phys[:, i] = data_loc[..., self.cons_idx[b"rho"]] * n0
                    # solution_phys[:, i] = calculate_n_cons(
                    #     data_loc,
                    ...
                    # u = u_0*U2/U1
                    solution_phys[:, i] = calculate_u_cons(data_loc, v0, self.cons_idx)
                elif phys_variable == b"Ei":
                    # Ei = m_0*u_0**2*U3/U1
                    solution_phys[:, i] = calculate_Ei_cons(
                        data_loc, v0**2 * m0, self.cons_idx
                    )
                    ...
                    solution_phys[:, i] = calculate_pi_cons(
                        data_loc, (2 / 3 / Mref) * n0 * T0 * q0, self.cons_idx
                    )
                    ...
                    solution_phys[:, i] = calculate_Ti_cons(
                        data_loc, T0, Mref, self.cons_idx
                    )
                    ...
                    solution_phys[:, i] = calculate_cs_cons(data_loc, v0, self.cons_idx)
                elif phys_variable == b"M":
                    # M = u/cs
                    solution_phys[:, i] = calculate_M_cons(data_loc, self.cons_idx)
                elif phys_variable == b"rhon":
                    # n_n = n_0*U5
                    solution_phys[:, i] = calculate_nn_cons(data_loc, n0, self.cons_idx)
                elif phys_variable == b"k":
                    # k = u_0**2*U6
                    solution_phys[:, i] = data_loc[..., self.cons_idx[b"k"]] * v0**2
                elif phys_variable == b"epsilon":
                    # epsilon = u_0**2/t0*U6
                    solution_phys[:, i] = (
                        data_loc[..., self.cons_idx[b"epsilon"]] * v0**2 / t0
                    )
                else:
                    raise KeyError(
                        f"Unknown variable {phys_variable}, go into the code and add this variable if you are sure"
                    )

                if len(data.shape) == 3:
                    self._solution_glob_phys = solution_phys.reshape(
                        (data.shape[0], data.shape[1], self.nphys)
                    )
                elif len(data.shape) == 2:
                    self._solution_simple_phys = solution_phys
                else:
                    raise ValueError(
                        "Something weird with the data shape of the solution"
                    )
            ...
            if len(data.shape) == 4:
                ...
                grad_phys = np.zeros(
                    (data.shape[0] * data.shape[1], self.nphys, self.ndim)
                )
                data_loc = data.reshape(
                    (data.shape[0] * data.shape[1], self.neq, self.ndim)
                )
                sol_loc = self.solution_glob.reshape(
                    (data.shape[0] * data.shape[1], self.neq)
                )
            ...
            for i in range(self.nphys):
                ...
                if phys_variable == b"rho":
                    ...
                    p0 = (2 / 3 / Mref) * n0 * T0 * q0

                    grad_phys[:, i, :] = calculate_grad_pi_cons(
                        sol_loc, data_loc, p0, l0, self.cons_idx
                    )
                elif phys_variable == b"pe":
                    ...
                    grad_phys[:, i, :] = calculate_grad_Ti_cons(
                        sol_loc, data_loc, T0, Mref, l0, self.cons_idx
                    )
                elif phys_variable == b"Te":
                    ...
                    grad_phys[:, i, :] = calculate_grad_M_cons(
                        sol_loc, data_loc, l0, self.cons_idx
                    )
                elif phys_variable == b"rhon":
                    ...
                    grad_phys[:, i, :] = (
                        data_loc[:, self.cons_idx[b"k"], :] * v0**2 / l0
                    )
                    # grad_phys[:, i, :] = calculate_grad_k_cons
                    #     data_loc, v0**2, l0, self.cons_idx
                    # )
                elif phys_variable == b"epsilon":
                    # grad(epsilon) = u0**2/t0/L0*grad(U6)
                    grad_phys[:, i, :] = (
                        data_loc[:, self.cons_idx[b"epsilon"], :] * v0**2 / t0 / l0
                    )

            if len(data.shape) == 4:
                self._gradient_glob_phys = grad_phys.reshape(
                    (data.shape[0], data.shape[1], self.nphys, self.ndim)
                )
            elif len(data.shape) == 3:
    ...
    def plot_overview_physical_difference(self, second_solution, n_levels=100):
        ...
        solutions_plot = np.zeros_like(self.solution_simple)
        solutions_plot[:, 0] = (
            self.solution_simple_phys[:, 0] - second_solution.solution_simple_phys[:, 0]
        )  # ne

        solutions_plot[:, 4] = (
            self.solution_simple_phys[:, 9] - second_solution.solution_simple_phys[:, 9]
        )  # M
        if self.neq > 2:
            solutions_plot[:, 2] = (
                self.solution_simple_phys[:, 6]
                - second_solution.solution_simple_phys[:, 6]
            )  # Ti
            solutions_plot[:, 3] = (
                self.solution_simple_phys[:, 7]
                - second_solution.solution_simple_phys[:, 7]
            )  # Te
        if self.neq > 4:
            solutions_plot[:, 1] = (
                self.solution_simple_phys[:, 10]
                - second_solution.solution_simple_phys[:, 10]
            )  # n_n
        if self.neq > 5:
            solutions_plot[:, 5] = (
                self.solution_simple_phys[:, 11]
                - second_solution.solution_simple_phys[:, 11]
            )  # k
    ...
    def plot_variables_overview(
        ...
        for variable in variable_list:
            if variable not in defined_variables:
                raise KeyError(
                    f"{variable} is not in the list of posible variables: {defined_variables}"
                )
        # additional triangulaton: since we have more than 3 points in each element, we can triangulate it
        # take any triangle from the mesh
        if self.mesh.connectivity_big is None:
        ...
        for i, (variable, label, limit, tick, tick_label, log) in enumerate(
            ...
            if variable == "n":
                ...
                if self.dk_parameters is None:
                    raise ValueError(
                        "Please, provide turbulent diffusion settings for the simulation"
                    )
            ...
            if var_to_plot > 2:
                ...
                axes[i % 2] = self.mesh.plot_full_mesh(
                    data,
                    ax=axes[i % 2],
                    log=log,
                    label=label,
    ...
    def define_magnetic_axis(self):
        ...
        self._r_axis, self._z_axis = self.mesh.vertices_glob[
            np.where(self.poloidal_flux_simple == self.poloidal_flux_simple.min())
        ][0]
    ...
    def define_minor_radii(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._a_simple[self.mesh.connectivity_glob.reshape(-1, 1).ravel()] = (
                self._a_glob.reshape(self._a_glob.shape[0] * self._a_glob.shape[1])
            )
        ...
        if which == "full":
            ...
            self._a_glob = calculate_a(
                self.mesh.vertices_glob[self.mesh.connectivity_glob],
                self.r_axis,
                self.z_axis,
            )
    ...
    def define_qcyl(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._qcyl_simple[self.mesh.connectivity_glob.reshape(-1, 1).ravel()] = (
                self._qcyl_glob.reshape(
                    self._qcyl_glob.shape[0] * self._qcyl_glob.shape[1]
                )
            )
            ...
            self._qcyl_glob = calculate_q_cyl(
                self.mesh.vertices_glob[self.mesh.connectivity_glob][:, :, 0],
                self.magnetic_field_glob[:, :, 0],
                self.magnetic_field_glob[:, :, 1],
                self.magnetic_field_glob[:, :, 2],
                self.a_glob,
            )
    ...
    def save_summary_line(self, save_folder, r_line, z_line, variable_list):
        ...
        values_on_line = self.calculate_variables_along_line(
            r_line, z_line, variable_list
        )
    ...
    def calculate_ohmic_source(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._ohmic_source_simple_simple = np.zeros(
                self.mesh.vertices_glob.shape[0]
            )
            self._ohmic_source_simple_simple[
                self.mesh.connectivity_glob.reshape(-1, 1).ravel()
            ] = self._ohmic_source.reshape(
                self.solution_glob.shape[0] * self.solution_glob.shape[1]
            )
        elif which == "full":
            if not self._combined_to_full:
                self.recombine_full_solution()
            self._ohmic_source = calculate_ohmic_source_cons(
                self.solution_glob,
                self.jtor_glob,
                self.parameters["physics"]["Mref"],
                self.parameters["adimensionalization"]["mass_scale"],
            ...
            self._ohmic_source_gauss = calculate_ohmic_source_cons(
                self.solution_gauss,
                self.jtor_gauss,
                self.parameters["physics"]["Mref"],
                self.parameters["adimensionalization"]["mass_scale"],
    ...
    def calculate_ionization_rate(self, which="simple"):
        ...
        if which == "simple":
            ...
            if "iz" not in self.atomic_parameters.keys():
                raise ValueError(
                    "Please, provide ionization atomic settings for the simulation"
                )
            if not self._simple_phys_initialized:
                print("Initializing physical solution first")
                self.init_phys_variables("simple")
            ...
            self._ionization_rate_simple[
                self.mesh.connectivity_glob.reshape(-1, 1).ravel()
            ] = self._ionization_rate.reshape(
                self.solution_glob.shape[0] * self.solution_glob.shape[1]
            )
            ...
            if not self._combined_to_full:
                self.recombine_full_solution()
            self._ionization_rate = calculate_iz_rate_cons(
                self.solution_glob,
                self.atomic_parameters["iz"],
                self.parameters["adimensionalization"]["temperature_scale"],
                self.parameters["adimensionalization"]["density_scale"],
                self.parameters["physics"]["Mref"],
            )
    ...
    def calculate_cx_rate(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._cx_rate_simple[self.mesh.connectivity_glob.reshape(-1, 1).ravel()] = (
                self._cx_rate.reshape(
                    self.solution_glob.shape[0] * self.solution_glob.shape[1]
                )
            )
        ...
        if which == "full":
            if not self._combined_to_full:
                self.recombine_full_solution()
            self._cx_rate = calculate_cx_rate_cons(
                self.solution_glob,
                self.atomic_parameters["cx"],
                self.parameters["adimensionalization"]["temperature_scale"],
                self.parameters["physics"]["Mref"],
            )
    ...
    def calculate_dnn(self, which="simple"):
        ...
        if which == "simple":
            ...
            if self.dnn_parameters is None:
                raise ValueError(
                    "Please, provide neutral diffusion settings for the simulation"
                )
            if "iz" not in self.atomic_parameters.keys():
                raise ValueError(
                    "Please, provide ionization atomic settings for the simulation"
            ...
            self._dnn_simple[self.mesh.connectivity_glob.reshape(-1, 1).ravel()] = (
                self._dnn.reshape(
                    self.solution_glob.shape[0] * self.solution_glob.shape[1]
                )
            )
    ...
    def calculate_dk(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._dk_simple[self.mesh.connectivity_glob.reshape(-1, 1).ravel()] = (
                self._dk_glob.reshape(self._dk_glob.shape[0] * self._dk_glob.shape[1])
            )
        ...
        if which == "full":
            ...
            self._dk_glob = calculate_dk_cons(
                self.solution_glob,
                self.dk_parameters,
                self.qcyl_glob,
                self.mesh.vertices_glob[self.mesh.connectivity_glob][:, :, 0]
    ...
    def calculate_mfp(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._mfp_simple[self.mesh.connectivity_glob.reshape(-1, 1).ravel()] = (
                self._mfp.reshape(
                    self.solution_glob.shape[0] * self.solution_glob.shape[1]
                )
            )
    ...
    def calculate_ionization_source(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._ionization_source_simple[
                self.mesh.connectivity_glob.reshape(-1, 1).ravel()
            ] = self._ionization_source.reshape(
                self.solution_glob.shape[0] * self.solution_glob.shape[1]
            )
        ...
        if which == "full":
            if not self._combined_to_full:
                self.recombine_full_solution()
            self._ionization_source = calculate_iz_source_cons(
                self.solution_glob,
                self.atomic_parameters["iz"],
                self.parameters["adimensionalization"]["temperature_scale"],
                self.parameters["adimensionalization"]["density_scale"],
        ...
        if which == "gauss":
            if self.solution_gauss is None:
                ...
                self.calculate_in_gauss_points()
            self._ionization_source_gauss = calculate_iz_source_cons(
                self.solution_gauss,
                self.atomic_parameters["iz"],
                self.parameters["adimensionalization"]["temperature_scale"],
                self.parameters["adimensionalization"]["density_scale"],
    ...
    def calculate_cx_source(self, which="simple"):
        ...
        if which == "simple":
            ...
            self._cx_source_simple[
                self.mesh.connectivity_glob.reshape(-1, 1).ravel()
            ] = self._cx_source.reshape(
                self.solution_glob.shape[0] * self.solution_glob.shape[1]
            )
        ...
        if which == "full":
            if not self._combined_to_full:
                self.recombine_full_solution()
            self._cx_source = calculate_cx_source_cons(
                self.solution_glob,
                self.atomic_parameters["cx"],
                self.parameters["adimensionalization"]["temperature_scale"],
                self.parameters["adimensionalization"]["density_scale"],
        ...
        if which == "gauss":
            if self.solution_gauss is None:
                print("Initializing values in gauss points first")
                self.calculate_in_gauss_points()
            self._cx_source_gauss = calculate_cx_source_cons(
                self.solution_gauss,
                self.atomic_parameters["cx"],
                self.parameters["adimensionalization"]["temperature_scale"],
                self.parameters["adimensionalization"]["density_scale"],
    ...
    def define_interpolators(self):
        ...
        if self._sample_interpolator is None:
            if self.mesh.mesh_parameters["element_type"] == "triangle":
                self._sample_interpolator = SoledgeHDG2DInterpolator(
                    self.mesh.vertices_glob,
                    np.ones_like(self.solution_glob[:, :, 0]),
                    self.mesh.connectivity_glob,
                    self.mesh.element_number,
                    self.mesh.reference_element["NodesCoord"],
                    self.mesh.mesh_parameters["element_type"],
                    self.mesh.p_order,
                    limit=False,
                )
                ...
                self._sample_interpolator = SoledgeHDG2DInterpolator(
                    ...
                    self.mesh.connectivity_glob,
                    self.mesh.element_number,
                    self.mesh.reference_element["NodesCoord1d"],
                    self.mesh.mesh_parameters["element_type"],
                    self.mesh.p_order,
                    limit=False,
                )
        ...
        for i in range(self.neq):
            self._solution_interpolators.append(
                SoledgeHDG2DInterpolator.instance(
                    self._sample_interpolator, self.solution_glob[:, :, i]
                )
            )
            grad = []
            grad.append(
                SoledgeHDG2DInterpolator.instance(
                    self._sample_interpolator, self.gradient_glob[:, :, i, 0]
                )
            )
            grad.append(
                SoledgeHDG2DInterpolator.instance(
                    self._sample_interpolator, self.gradient_glob[:, :, i, 1]
                )
            )
        ...
        for i in range(3):
            self._field_interpolators.append(
                SoledgeHDG2DInterpolator.instance(
                    self._sample_interpolator, self.magnetic_field_glob[:, :, i]
                )
            )

        # q_cylindrical
        self._qcyl_interpolator = SoledgeHDG2DInterpolator.instance(
            self._sample_interpolator, self.qcyl_glob
        )
    ...
    def n(self, r, z):
        ...
        if self._solution_interpolators is None:
            print(
                "Definition of interpolators will take some time for the initialization"
            )
            self.define_interpolators()
        # only fill needed field
        solution = np.zeros([1, self.neq])
        solution[:, self._cons_idx[b"rho"]] = self._solution_interpolators[
            self._cons_idx[b"rho"]
        ](r, z)

        return calculate_n_cons(
            solution,
            self.parameters["adimensionalization"]["density_scale"],
            self._cons_idx,
        )
    ...
    def ti(self, r, z):
        ...
        if solution[:, self._cons_idx[b"rho"]] == 0:
            return 0
        solution[:, self._cons_idx[b"Gamma"]] = self._solution_interpolators[
            self._cons_idx[b"Gamma"]
        ](r, z)
        solution[:, self._cons_idx[b"nEi"]] = self._solution_interpolators[
            self._cons_idx[b"nEi"]
        ](r, z)
    ...
    def te(self, r, z):
        ...
        solution[:, self._cons_idx[b"nEe"]] = self._solution_interpolators[
            self._cons_idx[b"nEe"]
        ](r, z)
    ...
    def nn(self, r, z):
        ...
        # only fill needed field
        solution = np.zeros([1, self.neq])
        solution[:, self._cons_idx[b"rhon"]] = self._solution_interpolators[
            self._cons_idx[b"rhon"]
        ](r, z)
        return calculate_nn_cons(
            solution,
            self.parameters["adimensionalization"]["density_scale"],
            self._cons_idx,
        )
    ...
    def cx_rate(self, r, z):
        ...
        if "cx" not in self.atomic_parameters.keys():
            raise ValueError(
                "Please, provide charge exchange atomic settings for the simulation"
            )
        if self._solution_interpolators is None:
            print(
                "Definition of interpolators will take some time for the initialization"
    ...
    def p_dyn(self, r, z):
        ...
        return calculate_pdyn_cons(
            solution,
            (2 / 3 / self.parameters["physics"]["Mref"])
            * self.parameters["adimensionalization"]["density_scale"]
            * self.parameters["adimensionalization"]["temperature_scale"]
    ...
    def ion_heat_flux_par_cond(self, r, z):
        ...
        return calculate_parallel_ion_heat_flux_par_cond_cons(
            ...
            Bz,
            Bt,
            self.parameters["physics"]["diff_pari"]
            / (
                self.parameters["adimensionalization"]["time_scale"] ** 3
                * self.parameters["adimensionalization"]["temperature_scale"] ** (7 / 2)
                / (
                    self.parameters["adimensionalization"]["density_scale"]
                    * self.parameters["adimensionalization"]["length_scale"] ** 4
                )
                / self.parameters["adimensionalization"]["mass_scale"]
            ),
            self.parameters["adimensionalization"]["temperature_scale"],
    ...
    def electron_heat_flux_par_cond(self, r, z):
        ...
        return calculate_parallel_electron_heat_flux_par_cond_cons(
            ...
            Bz,
            Bt,
            self.parameters["physics"]["diff_pare"]
            / (
                self.parameters["adimensionalization"]["time_scale"] ** 3
                * self.parameters["adimensionalization"]["temperature_scale"] ** (7 / 2)
                / (
