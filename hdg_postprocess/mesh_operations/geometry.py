...
def recombine_full_mesh(mesh):
    ...
    for i in range(mesh.n_partitions):
        mesh.global_state.n_elements = max(mesh.global_state.n_elements, mesh.raw_rest_mesh_data[i]["loc2glob_el"].max())
        mesh.global_state.n_vertices = max(mesh.global_state.n_vertices, mesh.raw_rest_mesh_data[i]["loc2glob_no"].max())
    ...
    for i in range(mesh.n_partitions):
        mesh.global_state.connectivity[mesh.raw_rest_mesh_data[i]["loc2glob_el"][~mesh.raw_ghost_elements[i].astype(bool).flatten()]] = (
            mesh.raw_rest_mesh_data[i]["loc2glob_no"][mesh.raw_connectivity[i][~mesh.raw_ghost_elements[i].astype(bool).flatten(), :]]
        )
        mesh.global_state.vertices[mesh.raw_rest_mesh_data[i]["loc2glob_no"], :] = mesh.raw_vertices[i][:, :]
...
def make_mask(mesh):
    ...
    mesh.derived_geometry.mask = Discrete2DMesh(
        mesh.vertices_glob, mesh.derived_geometry.connectivity_big, np.ones(mesh.derived_geometry.connectivity_big.shape[0]), limit=False, default_value=0
    )
...
def make_element_number_function(mesh):
    ...
    element_numbers = np.repeat(
        ...
        mesh.derived_geometry.connectivity_big.shape[0] // mesh.connectivity_glob.shape[0],
    )
    mesh.derived_geometry.element_locator = Discrete2DMesh(
        mesh.vertices_glob, mesh.derived_geometry.connectivity_big, element_numbers, limit=False, default_value=-1
    )
...
def calculate_gauss_volumes(mesh):
    ...
    mesh.derived_geometry.vertices_gauss = np.einsum("ij,kjh->kih", mesh.metadata.reference_element["N"], mesh.vertices_glob[mesh.connectivity_glob, :])
    J11_loc = np.einsum("ij,kj->ki", mesh.metadata.reference_element["Nxi"], mesh.vertices_glob[mesh.connectivity_glob, 0])
    J12_loc = np.einsum("ij,kj->ki", mesh.metadata.reference_element["Nxi"], mesh.vertices_glob[mesh.connectivity_glob, 1])
    J21_loc = np.einsum("ij,kj->ki", mesh.metadata.reference_element["Neta"], mesh.vertices_glob[mesh.connectivity_glob, 0])
    J22_loc = np.einsum("ij,kj->ki", mesh.metadata.reference_element["Neta"], mesh.vertices_glob[mesh.connectivity_glob, 1])
