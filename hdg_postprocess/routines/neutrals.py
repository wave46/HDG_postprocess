...
def calculate_dnn(ti, te, ne, dnn_params, atomic_parameters, kb, mD):
    ...
    if dnn_params["const"]:
        ...
        if dnn_params["ti_soft"]:
            ti = softplus(
                ti, dnn_params["ti_min"], dnn_params["ti_w"], dnn_params["ti_width"]
            )
        else:
...
def calculate_dnn_cons(
    solutions, dnn_params, atomic_parameters, kb, mD, T0, n0, Mref, L0, t0
):
    """
    ...
    if dnn_params["const"]:
        if len(solutions.shape) > 2:
            return dnn_params["dnn_max"] * np.ones(
                (solutions.shape[0], solutions.shape[1])
            )
        else:
        ...
        if len(solutions.shape) > 2:
            ...
            ti = (
                ...
                * (
                    ...
                    - 1 / 2 * solutions[:, :, 1] ** 2 / solutions[:, :, 0] ** 2
                )
            )
            ti = ti.flatten()
        ...
        sigma_iz = calculate_iz_rate_cons(
            solutions, atomic_parameters["iz"], T0, n0, Mref
        )
        ...
        if dnn_params["dnn_soft"]:
            dnn = double_softplus(
                dnn,
                dnn_params["dnn_min_adim"],
                dnn_params["dnn_max_adim"],
                dnn_params["dnn_w"],
                dnn_params["dnn_width"],
            )
        else:
