...
def calculate_plasma_resistivity_cons(solutions, Mref, mD, n0, L0, t0, ohmic_coef):
    ...
    if len(solutions.shape) > 2:
        eta = (
            ...
            * L0**2
            / t0**3
            / (2 / 3 / Mref * solutions[:, :, 3] / solutions[:, :, 0]) ** (3 / 2)
        )
    else:
        eta = (
            ohmic_coef
...
def get_simple_field_cons(y, y0, index):
    """
    Get the field from the conservative variable
    """
    return y[..., index] * y0
...
def calculate_grad_u_cons(solutions, gradients, u0, L0, cons_idx):
    ...
    res = (
        (
            ...
            * (
                -1
                * sol[:, cons_idx[b"Gamma"]][:, None]
                / sol[:, cons_idx[b"rho"]][:, None] ** 2
            )
            + grad[:, cons_idx[b"Gamma"], :] / sol[:, cons_idx[b"rho"]][:, None]
        )
        * u0
        / L0
    )
...
def calculate_grad_Ei_cons(solutions, gradients, E0, L0, cons_idx):
    ...
    res = (
        (
            ...
            * (
                -1
                * sol[:, cons_idx[b"nEi"]][:, None]
                / sol[:, cons_idx[b"rho"]][:, None] ** 2
            )
            + grad[:, cons_idx[b"nEi"], :] / sol[:, cons_idx[b"rho"]][:, None]
        )
        * E0
        / L0
    )
...
def calculate_grad_Ee_cons(solutions, gradients, E0, L0, cons_idx):
    ...
    res = (
        (
            ...
            * (
                -1
                * sol[:, cons_idx[b"nEe"]][:, None]
                / sol[:, cons_idx[b"rho"]][:, None] ** 2
            )
            + grad[:, cons_idx[b"nEe"], :] / sol[:, cons_idx[b"rho"]][:, None]
        )
        * E0
        / L0
    )
...
def calculate_pi_cons(solutions, p0, cons_idx):
    ...
    res = p0 * (
        sol[:, cons_idx[b"nEi"]]
        - 0.5 * sol[:, cons_idx[b"Gamma"]] ** 2 / sol[:, cons_idx[b"rho"]]
    )
    if dimensions is not None:
        res = res.reshape(dimensions[0], dimensions[1], dimensions[2])
    return res
...
def calculate_grad_pi_cons(solutions, gradients, p0, L0, cons_idx):
    ...
    res -= (
        grad[:, cons_idx[b"Gamma"], :]
        * sol[:, cons_idx[b"Gamma"]][:, None]
        / sol[:, cons_idx[b"rho"]][:, None]
    )
    res += (
        0.5
        * grad[:, cons_idx[b"rho"], :]
        * sol[:, cons_idx[b"Gamma"]][:, None] ** 2
        / sol[:, cons_idx[b"rho"]][:, None] ** 2
    )
    res *= p0 / L0
...
def calculate_Ti_cons(solutions, T0, Mref, cons_idx):
    ...
    res = (
        sol[:, cons_idx[b"nEi"]] / sol[:, cons_idx[b"rho"]]
...
def calculate_grad_Ti_cons(solutions, gradients, T0, Mref, L0, cons_idx):
    ...
    res += (
        grad[:, cons_idx[b"rho"], :]
        * (
            sol[:, cons_idx[b"Gamma"]] ** 2 / sol[:, cons_idx[b"rho"]] ** 3
            - sol[:, cons_idx[b"nEi"]] / sol[:, cons_idx[b"rho"]] ** 2
        )[:, None]
    )
    res -= (
        grad[:, cons_idx[b"Gamma"], :]
        * (sol[:, cons_idx[b"Gamma"]] / sol[:, cons_idx[b"rho"]] ** 2)[:, None]
    )
    res *= 2 / 3 / Mref * T0 / L0
...
def calculate_cs_cons(solutions, u0, cons_idx):
    ...
    res = u0 * np.sqrt(2 / 3 * res / sol[:, cons_idx[b"rho"]])
    if dimensions is not None:
        res = res.reshape(dimensions[0], dimensions[1], dimensions[2])
    return res
...
def calculate_grad_cs_cons(solutions, gradients, u0, L0, cons_idx):
    ...
    res = (
        ...
        * (
            (
                sol[:, cons_idx[b"Gamma"]] ** 2 / sol[:, cons_idx[b"rho"]]
                - (sol[:, cons_idx[b"nEe"]] + sol[:, cons_idx[b"nEi"]])
            )
            / sol[:, cons_idx[b"rho"]]
        )[:, None]
    )
    ...
    res += (grad[:, cons_idx[b"nEi"], :] + grad[:, cons_idx[b"nEe"], :]) / sol[
        :, cons_idx[b"rho"]
    ][:, None]
    res *= 1 / 3 / cs[:, None]
    res *= u0 * L0
...
def calculate_perp_ion_heat_wall_cons(
    solutions, gradients, diffusion, Br, Bz, Bt, n, E0, L0, cons_idx
):
    """
