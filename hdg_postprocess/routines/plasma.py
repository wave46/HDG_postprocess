...
def _flatten_gradient_nodes(gradients):
    if gradients.ndim > 3:
        ...
        return gradients.reshape(
            shape[0] * shape[1], gradients.shape[2], gradients.shape[3]
        ), shape
...
def _ratio_gradient(grad_numerator, grad_denominator, numerator, denominator):
    return (
        grad_numerator / denominator[:, None]
        - grad_denominator * (numerator / denominator**2)[:, None]
    )
...
def calculate_plasma_resistivity_cons(
    solutions, Mref, mD, n0, L0, t0, ohmic_coef, Zeff
):
    """
...
def calculate_ohmic_source_cons(
    solutions, jtor, Mref, mD, n0, L0, t0, ohmic_coef, Zeff
):
    """
    calculates phmic heating source resistivity based on conservatives values
    """

    eta = calculate_plasma_resistivity_cons(
        solutions, Mref, mD, n0, L0, t0, ohmic_coef, Zeff
    )
    return eta * jtor**2


def calculate_variable_cons(
    solutions,
    variable,
    adimensionalization,
    cons_idx,
    ...
    if variable == "n":
        return calculate_n_cons(
            solutions, adimensionalization["density_scale"], cons_idx
        )
    elif variable == "nn":
        ...
        return calculate_Ei_cons(
            solutions,
            adimensionalization["mass_scale"] * adimensionalization["speed_scale"] ** 2,
            cons_idx,
        )
    elif variable == "Ee":
        return calculate_Ee_cons(
            solutions,
            adimensionalization["mass_scale"] * adimensionalization["speed_scale"] ** 2,
            cons_idx,
        )
...
def calculate_grad_u_cons(solutions, gradients, u0, L0, cons_idx):
    ...
    res = _ratio_gradient(
        _grad(grad, b"Gamma", cons_idx), _grad(grad, b"rho", cons_idx), gamma, rho
    )
    res *= u0 / L0
    return _reshape_vector(res, shape)
...
def calculate_grad_Ei_cons(solutions, gradients, E0, L0, cons_idx):
    ...
    res = _ratio_gradient(
        _grad(grad, b"nEi", cons_idx), _grad(grad, b"rho", cons_idx), nEi, rho
    )
    res *= E0 / L0
    return _reshape_vector(res, shape)
...
def calculate_grad_Ee_cons(solutions, gradients, E0, L0, cons_idx):
    ...
    res = _ratio_gradient(
        _grad(grad, b"nEe", cons_idx), _grad(grad, b"rho", cons_idx), nEe, rho
    )
    res *= E0 / L0
    return _reshape_vector(res, shape)
...
def calculate_Ti_cons(solutions, T0, Mref, cons_idx):
    ...
    res = (
        ...
        / Mref
        * T0
        * (_sol(sol, b"nEi", cons_idx) / rho - 0.5 * gamma**2 / rho**2)
    )
    return _reshape_scalar(res, shape)
...
def calculate_grad_Ti_cons(solutions, gradients, T0, Mref, L0, cons_idx):
    ...
    res += (
        _grad(grad, b"rho", cons_idx) * ((gamma**2 / rho**3) - (nEi / rho**2))[:, None]
    )
...
def calculate_grad_Ti_par_cons(
    solutions, gradients, Br, Bz, Bt, T0, Mref, L0, cons_idx
):
    """
...
def calculate_Te_cons(solutions, T0, Mref, cons_idx):
    ...
    sol, shape = _flatten_solution_nodes(solutions)
    res = (
        2 / 3 / Mref * T0 * (_sol(sol, b"nEe", cons_idx) / _sol(sol, b"rho", cons_idx))
    )
    return _reshape_scalar(res, shape)
...
def calculate_grad_Te_par_cons(
    solutions, gradients, Br, Bz, Bt, T0, Mref, L0, cons_idx
):
    """
...
def calculate_cs_cons(solutions, u0, cons_idx):
    ...
    gamma = _sol(sol, b"Gamma", cons_idx)
    thermal = (
        _sol(sol, b"nEi", cons_idx) + _sol(sol, b"nEe", cons_idx) - 0.5 * gamma**2 / rho
    )
...
def calculate_grad_cs_cons(solutions, gradients, u0, L0, cons_idx):
    ...
    res = (
        _grad(grad, b"rho", cons_idx)
        * ((gamma**2 / rho - (nEe + nEi)) / rho**2)[:, None]
    )
    res -= _grad(grad, b"Gamma", cons_idx) * (gamma / rho**2)[:, None]
    res += (_grad(grad, b"nEi", cons_idx) + _grad(grad, b"nEe", cons_idx)) / rho[
        :, None
    ]
    res *= 1 / 3 / cs[:, None]
    res *= u0 / L0
...
def calculate_eps_cons(solutions, eps0, cons_idx):
    """
    calculates turbulent dissipation value based on conservatives values
    """
    dimensions = None
    if len(solutions.shape) > 2:
        dimensions = solutions.shape
    ...
    res = sol[:, cons_idx[b"epsilon"]] * eps0
    if dimensions is not None:
        res = res.reshape(dimensions[0], dimensions[1])
    return res
...
def calculate_parallel_ion_heat_flux_par_conv_cons(
    solutions, n0, T0, Mref, kb, mD, u0, cons_idx
):
    """
...
def calculate_parallel_ion_heat_flux_par_cond_cons(
    solutions, gradients, Br, Bz, Bt, q0, T0, Mref, L0, Tmax, cons_idx
):
    """
    ...
    grad_ti_par = calculate_grad_Ti_par_cons(
        sol, grad, Br_res, Bz_res, Bt_res, T0, Mref, L0, cons_idx
    )
    res = -q0 * ti ** (5 / 2) * grad_ti_par
    return _reshape_scalar(res, shape)


def calculate_parallel_ion_heat_flux_par_cons(
    solutions, gradients, Br, Bz, Bt, n0, q0, T0, Mref, kb, mD, u0, L0, Tmax, cons_idx
):
    """
    calculates parallel conductive ion heat flux value based on conservatives values
    ...
    q_iconv = calculate_parallel_ion_heat_flux_par_conv_cons(
        sol, n0, T0, Mref, kb, mD, u0, cons_idx
    )
    q_icond = calculate_parallel_ion_heat_flux_par_cond_cons(
        sol, grad, Br_res, Bz_res, Bt_res, q0, T0, Mref, L0, Tmax, cons_idx
    )
...
def calculate_parallel_electron_heat_flux_par_conv_cons(
    solutions, n0, T0, Mref, kb, u0, cons_idx
):
    """
...
def calculate_parallel_electron_heat_flux_par_cond_cons(
    solutions, gradients, Br, Bz, Bt, q0, T0, Mref, L0, Tmax, cons_idx
):
    """
    ...
    grad_te_par = calculate_grad_Te_par_cons(
        sol, grad, Br_res, Bz_res, Bt_res, T0, Mref, L0, cons_idx
    )
...
def calculate_parallel_electron_heat_flux_par_cons(
    solutions, gradients, Br, Bz, Bt, n0, q0, T0, Mref, kb, u0, L0, Tmax, cons_idx
):
    """
    calculates parallel conductive electron heat flux value based on conservatives values
    ...
    q_econv = calculate_parallel_electron_heat_flux_par_conv_cons(
        sol, n0, T0, Mref, kb, u0, cons_idx
    )
    q_econd = calculate_parallel_electron_heat_flux_par_cond_cons(
        sol, grad, Br_res, Bz_res, Bt_res, q0, T0, Mref, L0, Tmax, cons_idx
    )
...
def calculate_particle_perp_flux_wall_cons(
    solutions, gradients, diffusion, Br, Bz, Bt, n, n0, L0, cons_idx
):
    """
...
def calculate_perp_ion_heat_wall_cons(
    solutions, gradients, diffusion, Br, Bz, Bt, n, n0, E0, L0, cons_idx
):
    """
...
def calculate_perp_electron_heat_wall_cons(
    solutions, gradients, diffusion, Br, Bz, Bt, n, n0, E0, L0, cons_idx
):
    """
    calculates perpendicular ion heat flux on the wall with normal n value based on conservatives values
...
def calculate_ion_heat_flux_wall_bc_cons(
    solutions, gamma_i, Br, Bz, Bt, n, n0, u0, T0, Mref, kb, mD, cons_idx
):
    """
    calculates ion heat flux on the wall with normal n value based on conservatives values
    diffusion dimensional (assuming Dperp only)
    """
    sol, Br_res, Bz_res, Bt_res, shape = _flatten_solution_with_fields(
        solutions, Br, Bz, Bt
    )
...
def calculate_electron_heat_flux_wall_bc_cons(
    solutions, gamma_e, Br, Bz, Bt, n, n0, u0, T0, Mref, kb, cons_idx
):
    """
