...
class SoledgeHDG2DInterpolator:
    ...
    def evaluate(self, x, y):
        ...
        if (x, y) in self._hashed_shape_functions.keys():
            ...
            if element_number == -1:
                ...
                element_vertices = self._vertex_coords[
                    self._connectivity[element_number, :]
                ]
                ...
                if self._element_type == "triangle":
                    shape_functions = (
                        orthopoly2D(xieta[0], xieta[1], self._p_order)
                        @ self._inv_vandermonde
                    )
                elif self._element_type == "quadrilateral":
                    shape_functions = shapefunctions_quads(
                        xieta[0], xieta[1], self._p_order, self._inv_vandermonde
                    )
    ...
    def __setstate__(self, state):
        self._vertex_data, self._element_number, self._limit, self._default_value = (
            state
        )
    ...
    def instance(cls, instance, vertex_data=None, limit=None, default_value=None):
        ...
        if vertex_data is None:
            ...
            m._vertex_data = np.array(vertex_data, dtype=np.float64).reshape(
                (m._connectivity.shape[0], m._connectivity.shape[1])
            )
            if (
                m._vertex_data.ndim != 2
                or m._vertex_data.shape[0] != instance._vertex_data.shape[0]
            ):
                raise ValueError(
                    "Vertex_data dimensions are incompatible with the number of vertices in the instance ({} vertices).".format(
                        instance._vertex_data.shape[0]
                    )
                )
...
def xieta_element(x, y, vertices_element, eltype):
    ...
    if eltype == "triangle":
        ...
        # compute jacobian
        J[0, 0] = (
            vertices_element[1, 0]
            + vertices_element[2, 0]
            - vertices_element[0, 0]
            - vertices_element[3, 0]
        )
        J[0, 1] = (
            vertices_element[1, 1]
            + vertices_element[2, 1]
            - vertices_element[0, 1]
            - vertices_element[3, 1]
        )
        J[1, 0] = (
            vertices_element[2, 0]
            + vertices_element[3, 0]
...
def xieta_element_precise(x, y, vertices_element, p_order, inv_vandermonde, eltype):
    ...
    if eltype == "triangle":
        [x_back, y_back] = (
            orthopoly2D(xieta0[0], xieta0[1], p_order) @ inv_vandermonde
        ) @ vertices_element
    elif eltype == "quadrilateral":
        shape_functions = shapefunctions_quads(
            xieta0[0], xieta0[1], p_order, inv_vandermonde
        )
        [x_back, y_back] = shape_functions[:, 0] @ vertices_element
    if (
        np.sqrt(((x_back - x) ** 2 + (y_back - y) ** 2))
        < tol * np.sqrt(x**2 + y**2) + 1e-14
    ):
        return xieta0
    else:
        for i in range(maxit):
            if (
                ...
                return xieta0
            if eltype == "triangle":
                _, dp_dxi, dp_deta = orthopoly2D_deriv_xieta(
                    xieta0[0], xieta0[1], p_order
                )
        ...
        if np.sqrt(((x_back - x) ** 2 + (y_back - y) ** 2)) > (
            tol * np.sqrt(x**2 + y**2) + 1e-14
        ):
...
def Vandermonde_LP(p_order, coord):
    ...
    if nsd == 1:
        if N != p_order + 1:
            raise ValueError(
                "The number of polynomials does not coincide with the number of nodes"
            )
        for i in range(N):
...
permutations_quads[3] = np.array(
    [1, 5, 6, 2, 12, 13, 14, 7, 11, 15, 16, 8, 4, 10, 9, 3], dtype=int
)
permutations_quads[4] = (
    np.array(
        [
            1,
            5,
