...
def load_HDG_solution_from_file(
    solpath, solname_base, meshpath, meshname_base, n_partitions
):
    """
    ...
    raw_solutions = (
        []
    )  # raw solutions "u" for every node for every element as it comes from HDG
    raw_solutions_skeleton = (
        []
    )  # raw solutions "u_tilde" on a skeleton mesh as it comes from HDG
    raw_gradients = (
        []
    )  # raw gradients "q" for every node for every element as it comes from HDG
    raw_equilibriums = (
        []
    )  # list with dictionaries with plasma current, magnetic field, poloidal flux
    raw_solution_boundary_infos = (
        []
    )  # list with dictionaries with info about boundary flags and exterior faces (not sure if it will be needed later)

    parameters = (
        {}
    )  # parameters of the simulation which do not change between partitions
    ...
    for n_partition in range(1, n_partitions + 1):
        ...
        equilibrium["poloidal_flux"] = solution_file[
            "magnetic_psi"
        ].T  # this might be corrupted or normalized
        ...
        solution_boundary_data["exterior_faces"] = (
            solution_file["exterior_faces"].T.astype(int) - 1
        )
...
def load_HDG_mesh_from_file(meshpath, meshname_base, n_partitions):
    ...
    mesh_parameters = (
        {}
    )  # parameters of the mesh which do not change between partitions
    raw_vertices = []
    raw_connectivity = []
    raw_connectivity_boundary = []
    raw_mesh_numbers = (
        []
    )  # list containing dictionaries with number of elements, external faces, faces, nodes in the given mesh
    raw_boundary_flags = (
        []
    )  # list with boundary flags to_add: boundary flags description
    raw_ghost_elements = (
        []
    )  # list with flag if ghost element, then True, if not ghost then False
    raw_ghost_faces = (
        []
    )  # list with flag if ghost face, then True, if not ghost then False
    raw_rest_mesh_data = (
        []
    )  # list with the rest mesh data not well understood and used so far
